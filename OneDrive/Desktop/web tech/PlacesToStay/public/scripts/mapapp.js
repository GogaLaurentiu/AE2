...
map.setView([39.2477, 21.1614], 13);
...
function accSearchMap(mapObject, location, type = null) {
            ...
            const response = fetch(`/acc/search/${type}/${location}`)
                            .then(response => {
                                ...
                                response.json().then( accInfo => {
                                    ...
                                    const message = `FOUND RESULTS FOR Accommodations with the Type: ${type} and Location: ${location}`;
    ...
    console.log("SEARCHING WITHOUT A TYPE");
    try {
                        ...
                        .then(response => {
                            ...
                            response.json().then( accInfo => {
                                ...
                                if(accInfo){
                                    ...
                                    const message = `FOUND RESULTS FOR Accommodations with the Location: ${location}`;
                                    messageLocation.innerHTML = message;
                                    return;
                                }
                            });
