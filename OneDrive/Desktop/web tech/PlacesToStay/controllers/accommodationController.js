...
class AccommodationController {
    ...
    async findAll(req, res){
        try {
            ...
            if(acc == null) {
                res.status(404).json({error: `No accommodations saved in the Database`});
            } else {
                res.json(acc);    
            }
        } catch (e) {
    ...
    async findAccByLocationType(req, res) {
        try {
            ...
            if(acc == null) {
                res.status(404).json({error: `Unable to Locate any accommodation in ${req.params.location} with Type of: ${req.params.type}`});
            } else {
                res.json(acc);    
            }
        } catch (e) {
